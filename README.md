...
export TRPG_BASE_URL=https://api.kimi.com/coding/v1
export TRPG_MODEL=kimi-for-coding
export TRPG_HEADERS='{"User-Agent":"claude-code/1.0.0","X-Client-Name":"claude-code"}'
export TRPG_STREAM_USAGE=false
npm run web
```

> 项目当前主用 Kimi 的 coding API(`kimi-for-coding`),它需要伪装成 coding agent
> (`TRPG_HEADERS`)并禁用 stream usage 元数据(`TRPG_STREAM_USAGE=false`)。
> 切到其它 OpenAI-compatible endpoint(vLLM / Ollama / DeepSeek 直连等)时,
> 把 `TRPG_BASE_URL`/`TRPG_MODEL` 替换成对应值,`TRPG_HEADERS` 通常可以省掉。

支持的环境变量:
- `TRPG_API_KEY` (必填)
- `TRPG_BASE_URL` (必填,OpenAI-compatible endpoint)
- `TRPG_MODEL` (默认 `kimi-for-coding`)
...
切换其它 OpenAI 兼容 endpoint 时,移除 `headers` 和 `streamUsage`,
把 `baseUrl`/`model` 改成对应的值即可:
...
- **kimi-for-coding** (项目当前默认,通过 Kimi coding API)
- DeepSeek-V3 / R1
