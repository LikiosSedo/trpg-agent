...
暴力后果由两个独立的计时器控制，互不干扰：

```
发现倒计时（alert.delay）：固定，暴力发生时确定，不受逃跑影响
追击倒计时（alert.chaseDelay）：动态，取决于玩家当前位置，不叠加
```
...
玩家攻击 NPC
  ...
  → startCombat()
  → 创建 violence_alert：
    delay = 基础 5 轮 + 时间修正 - 目击者修正
    chaseDelay = 0（初始，每轮更新）
```
...
每轮开始时，根据玩家**当前位置**设定 `chaseDelay`（不叠加，只看最终状态）：

| 玩家位置 | chaseDelay | 需要 canTrack | 说明 |
|---------|-----------|--------------|------|
| 暴力现场 | 0 | 否 | 立即对峙 |
| 同区域其他子地点 | 1 | 否 | 镇内很快追到 |
| 不同区域（可达） | 2 | 是 | 跨区域追踪 |
...
**关键**：玩家来回跑不叠加延迟。从森林跑回镇上，chaseDelay 从 2 变回 1。
...
**发现阶段做两件事**：
1. 响应者移动到受害者处（`moveNPC`），标记 `discoveryTurn`
2. 触发信任度传播（`propagateViolenceTrust`）

**不触发战斗。**

#### 第 N + chaseDelay 轮：阶段 2 — 追击到达

**条件**：`discoveryTurn` 存在 且 `turnCount - discoveryTurn >= chaseDelay`

**追击到达时**：
1. 响应者移动到**玩家当前位置**
2. 设置 `pendingCombatInterrupt`
3. DM 叙事后自动触发战斗

**追击预警**（到达前 1 轮，chaseDelay >= 2 时）：
```
韩猛："站住！懦夫！"
艾琳娜："跑不掉的。"
格雷格：沉重脚步声 + 杀意
...
**发现前**：
```
[世界事件预告：{受害者}的遭遇即将被发现，约{N}轮后]
```

**发现时（chaseDelay = 0，玩家在场）**：
```
[世界事件：{响应者}赶到现场，发现{受害者}{状态}！
描写到达——脚步声、质问、愤怒。不描写攻击，下一轮战斗开始。]
```

**发现时（chaseDelay > 0，玩家不在场）**：
```
[世界事件：{受害者}的遭遇被发现了。
你不在现场——让玩家感受远处的不安氛围。{响应者}正在追踪你。]
```

**追击到达**：
```
[战斗触发：{响应者}追上了你！用1-2句描写出现的瞬间和战斗开场。]
```
