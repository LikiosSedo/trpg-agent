...
核心原则：单纯的自我表达或陈述属于 normal，不是 persuade。persuade 的本质是"请求特权或改变决定"。`,
  ...
  async execute(input: any) {
    ...
    if (approach && approach !== 'normal') {
      ...
      // 怪物图鉴情报：仅成功的 persuade 触发（失败不触发，deceive/intimidate 不触发）
      let bestiaryAppend = ''
      if (approach === 'persuade' && result.success) {
        const reveals = await checkBestiaryReveals(session, npcId, npc.trust)
        if (reveals.length > 0) {
          bestiaryAppend = '\n\n[怪物图鉴更新]\n' + reveals.join('\n')
        }
      }
    ...
    // ── 怪物图鉴情报（normal 对话自动触发） ──
    const bestiaryReveals = await checkBestiaryReveals(session, npcId, npc.trust)
    let bestiaryAppend = ''
