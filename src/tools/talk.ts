...
// ─── Talk 数据捕获（供 NPC 记忆提取用） ───
interface TalkDataEntry { npcName: string; playerMessage: string; talkOutput: string }
const pendingTalkData: TalkDataEntry[] = []

/** 消费本轮所有 Talk 调用的数据（NPC 记忆提取器使用） */
export function consumePendingTalkData(): TalkDataEntry[] {
  const result = [...pendingTalkData]
  pendingTalkData.length = 0
  return result
}
...
核心原则：单纯的自我表达或陈述属于 normal，不是 persuade。persuade 的本质是"请求特权或改变决定"。`,
  ...
  async execute(input: any) {
    ...
    const talkOutput = [
      ...
      autoTrustBumped ? `[系统：信任已自动 +1，无需再调 ChangeTrust]` : '',
      autoTrustBumped
        ? ''
    ...
    // 捕获 Talk 数据供 NPC 记忆提取器使用
    pendingTalkData.push({ npcName: npc.name, playerMessage: message, talkOutput })

    return { output: talkOutput }
  },
}
